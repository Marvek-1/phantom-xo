...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "PUT") {
      ...
      if (!laneId) return new Response(JSON.stringify({ error: "lane_id required" }), { status: 400, headers: withCorsHeaders({ "Content-Type": "application/json" }) });

      const { data: lane } = await db.from("data_lanes").select("*").eq("id", laneId).maybeSingle();
      if (!lane) return new Response(JSON.stringify({ error: "Lane not found" }), { status: 404, headers: withCorsHeaders({ "Content-Type": "application/json" }) });
